...
# Sort/separate in terms of X_SrO for peak logic
peak_idx = np.argmax(y_bin)
T_max_bin = y_bin[peak_idx]
X_peak_bin = x_bin[peak_idx]

# Left Branch (CaO-rich) in terms of X_SrO
x_left = x_bin[:peak_idx + 1]
T_left = y_bin[:peak_idx + 1]
# Right Branch (SrO-rich) in terms of X_SrO
x_right = x_bin[peak_idx:]
T_right = y_bin[peak_idx:]

# Convert coordinates to X_CaO
x_bin_CaO = 1.0 - x_bin
x_left_CaO = 1.0 - x_left
...
x_liq_CaO = 1.0 - liq_pts[:, 0]
y_liq = liq_pts[:, 1]

# =========================================================================
# 3. EXPERIMENTAL DATA (Risold 1997 & Jacob 2000)
# =========================================================================
# Risold data et al. (1997) - AGAPT. Composition given in x_CaO
exp_data_AGAPT = [
...
exp_x_agap_CaO = [pt[1] for pt in exp_data_AGAP]
...
X_l_interp = np.interp(T_grid_bin, T_left[sort_l], x_left_CaO[sort_l])
X_r_interp = np.interp(T_grid_bin, T_right[sort_r], x_right_CaO[sort_r])
...
# Liquidus has lower X_CaO than solidus for a given T in the coexistence region
ax.fill_betweenx(T_grid_high, X_liq_interp, X_sol_interp, color="#e377c2", alpha=0.3)

# C. Overlaying Experimental Points
ax.scatter(exp_x_agapt_CaO, exp_t_agapt, color="#d62728", marker="o", facecolors="none", 
           edgecolors="#d62728", s=40, zorder=5, label="AGAPT Data (Risold et al., 1997)")
ax.scatter(exp_x_agap_CaO, exp_t_agap, color="green", marker="s", s=45, zorder=5, 
           label="AGAP Data (Jacob et al., 2000)")

# D. Annotations of Singular Points and Phase Fields
# Melting points
ax.plot(1.0, 3222.5, marker="o", color="black", markersize=4) # CaO is at X_CaO = 1.0
ax.text(0.98, 3122.5, r"T$_{\mathrm{m}}$(CaO) = 3222.5 K (2949.4 °C)", fontsize=8.5, ha="right", va="center")

ax.plot(0.0, 2870.4, marker="o", color="black", markersize=4) # SrO is at X_CaO = 0.0
ax.text(0.02, 2755.0, r"T$_{\mathrm{m}}$(SrO) = 2870.4 K (2597.3 °C)", fontsize=8.5, ha="left", va="bottom")

# Miscibility Gap Critical Point
ax.plot(X_peak_bin_CaO, T_max_bin, marker="v", color="darkblue", markersize=5)
ax.text(X_peak_bin_CaO, T_max_bin + 25, rf"T$_c$ = {T_max_bin:.1f} K" + "\n" + rf"X$_{{\mathrm{{CaO}}}}$ = {X_peak_bin_CaO:.3f}", 
        fontsize=8.5, color="darkblue", ha="center", va="bottom")

# Phase field labels
# Note: Liquid is wider towards low X_CaO (SrO-rich), so place LIQUID label at X_CaO = 0.30
ax.text(0.30, 3140, "LIQUID", fontsize=11, fontweight="bold", ha="center", color="darkred")
# Move Homogeneous Solid Solution to X_CaO = 0.65 to mirror X_SrO = 0.35
ax.text(0.65, 1900, "Homogeneous Solid Solution\nHalite (Ca,Sr)O", fontsize=10, ha="center", 
        bbox=dict(boxstyle="round,pad=0.4", facecolor="white", edgecolor="#cccccc", alpha=0.9))
ax.text(X_peak_bin_CaO, 970, "Miscibility Gap\nHalite#1 + Halite#2", fontsize=10.5, ha="center", color="darkblue",
...
# =========================================================================
# 5. AXES CONFIGURATION (Double axes - Bottom: CaO, Top: SrO)
# =========================================================================
ax.set_xlabel(r"Mole Fraction of CaO, $X_{\mathrm{CaO}}$")
...
ax2.set_xlabel(r"Mole Fraction of SrO, $X_{\mathrm{SrO}}$")
...
# Legend (placed on the center left where it's SrO-rich and very empty)
...
# Save figure
out_filename = "diagrama_CaSrO_CaO.png"
...
# Liquidus and Solidus
ax_z.plot(x_liq_CaO, y_liq, color="#d62728", linestyle="-", linewidth=1.5, label="Calculated Liquidus")
ax_z.plot(x_sol_CaO, y_sol, color="#e377c2", linestyle="--", linewidth=1.2, label="Calculated Solidus")

# B. Shading of Phase Regions
# Shading L + Halite coexistence region
ax_z.fill_betweenx(T_grid_high, X_liq_interp, X_sol_interp, color="#e377c2", alpha=0.3)

# C. Annotations of Melting Points
ax_z.plot(1.0, 3222.5, marker="o", color="black", markersize=4)
ax_z.text(0.98, 3122.5, r"T$_{\mathrm{m}}$(CaO) = 3222.5 K (2949.4 °C)", fontsize=8.5, ha="right", va="center")

# Melting of SrO is at 2870.4 K (2597.3 °C) which is just below the bottom limit of 2600 °C (2873.15 K)
ax_z.plot(0.0, 2870.4, marker="o", color="black", markersize=4)

# Phase field labels
ax_z.text(0.20, 3140, "LIQUID", fontsize=11, fontweight="bold", ha="center", color="darkred")
# Solid region is wider on the right (higher X_CaO, higher melting temp), so place homogeneous solution at X_CaO = 0.70
ax_z.text(0.70, 2940, "Homogeneous Solid Solution\nHalite (Ca,Sr)O", fontsize=10, ha="center", 
          bbox=dict(boxstyle="round,pad=0.4", facecolor="white", edgecolor="#cccccc", alpha=0.9))

# D. AXES CONFIGURATION (Double axes - Bottom: CaO, Top: SrO)
ax_z.set_xlabel(r"Mole Fraction of CaO, $X_{\mathrm{CaO}}$")
...
ax2_z.set_xlabel(r"Mole Fraction of SrO, $X_{\mathrm{SrO}}$")
...
# F. Inset Axis for High-Resolution Liquidus-Solidus Separation at X_CaO = 0.5 (which is also X_SrO = 0.5)
axins = ax_z.inset_axes([0.08, 0.08, 0.38, 0.38])
axins.plot(x_liq_CaO, y_liq, color="#d62728", linestyle="-", linewidth=1.5)
axins.plot(x_sol_CaO, y_sol, color="#e377c2", linestyle="--", linewidth=1.2)
axins.fill_betweenx(T_grid_high, X_liq_interp, X_sol_interp, color="#e377c2", alpha=0.3)
...
# Save zoomed figure
out_filename_zoom = "diagrama_CaSrO_CaO_zoom.png"
